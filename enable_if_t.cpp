...
template <typename T> std::enable_if_t<std::is_same_v<T, int>> print0(T value) {
...
template <typename T,
          std::enable_if_t<std::is_same_v<T, size_t>, size_t> size1 = 10>
void print1(T size0) {
...
template <typename T, typename Tag = int> class A {
  T data0;
  Tag data1;
